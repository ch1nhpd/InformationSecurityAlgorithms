...
int cmpA(int X[], int Y[], int sizeArr)
{
	for(int i = sizeArr-1; i>=0;--i)
	{
		if(X[i]<Y[i])
...
int addInFp(int A[], int B[], int C[], int P[], int sizeArr, int w)
{
	int e = mulAdd(A,B,C,sizeArr,w);
	//printf("e = %d",e);
	//printArr(C,sizeArr);
	if(e==1 || cmpA(C,P,sizeArr) == 1 )
	{
		e= mulSub(C,P,C,sizeArr,w);
	}
	return e;
}
...
int main()
{
	...
	int e = addInFp(A,B,C,P,sizeArr,w);
